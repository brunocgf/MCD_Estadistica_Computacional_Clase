...
```{r }
ggplot(perfiles_int) +
  geom_segment(aes(y = price, yend = price, x = Int_inf, xend = Int_sup), size = .5) +
  geom_point(aes(x = perfil, y = price), size = 1) +
  facet_wrap(how~.) +
...
En la categoria de teabag la mayor tasa de compra son los precios "private label y unknown" que son .72 aproximadamente con un intervalo de confianza entre .10 y 1.34. Lo cual nos indica que este patron de comportamiento es alto y significa porque los usuarios de tea_bag compran mas a estos precios. Por otro lado, en est misma categoría lo que menos compran es tea precio p_upscale.
...
Por útimo, en la categoría unpackaged la mayor tasa de compra es a precio p_upscale a una tasa de casi el doble .89 con un intervalo de confianza de .68 y 1.28, en el caso de la menor tasa de compra se obtuvó que la menor fue a precio p_private_label.
Del mismo modo, podemos que tanta variación hay en los intervalos calculados y los que tienen una longuitud (segmento) muy ammplio no son tan confiables, se esperaría que la longuitud fuera lo menos posible para tener mayor certeza de que valor tomaría la variable estimada.
En conclusión pareciera que el mejor comportamiento de compra se desarrolla en la categoría p_unpackaged a un precio upscale, po lo que se recomendaría trabajar en dicho segmento, porque ahí la gente tiene un perfil de compra mas agudo que en los demás casos. 
...
map_dbl(enlace_boot_rep, sd)[2:3]
```
Con la simulación MonteCarlo obtuvimos que conforme va aumentando el número de replicaciones(simulaciones) el error de monte carlo va dismunuyendo, esto que tiende a cero y por eso los intervalos que obtuvimos son cada vez más pequeños y cercanos a cero.
...
```{r poisson grafica}
  ggplot(poiss_rep_int) +
    geom_pointrange(aes(x = reorder(muestra,theta),
                        ...
                        ymax = superior),
                    size=0.2) +
    geom_hline(yintercept = exp(-2.5*2),color="pink") +
    facet_grid(metodo~.)
```
...
En el caso del conteo rápido es posible evaluar la cobertura del intervalo de confianza bootstrap (tarea 5) usando los resultados de elecciones pasadas, para ello usaremos los resultados de las elecciones 2006 (datos `election_2006` del paquete `estcomp`) repetirás los siguientes dos pasos 100 veces (asegurate de que tu ejercicio de simulación sea replicable):
...
```{r}
time1<-system.time(rerun(10000, sim_binn(0.7,20)))
time1
```
```{r}
time2<-system.time(rerun(10000, sim_binn_rec(0.7,20)))
time2
```
En la recursiva de la binomial el tiempo de ejecución es menor por el while que tiene la función, ya que si no se cumple la condición de U>=P automaticamente se va directo a la siguiente r deseada.
...
```{r}
hist_geom<-rerun(1000, sim_geometrica(0.7,20)) %>% flatten_dbl()
qplot(hist_geom, binwidth = 1)+labs(title="Histograma de geometrica con 1000 replicaciones")
```

```{r}
hist_bin<-rerun(1000, sim_binn(0.7,20)) %>% flatten_dbl()
qplot(hist_bin, binwidth = 1)+labs(title="Histograma de binomial negativa con 1000 replicaciones")
```
