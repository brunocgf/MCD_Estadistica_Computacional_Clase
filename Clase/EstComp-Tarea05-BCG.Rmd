...
* La tabla `sample_2006` en el paquete `estcomp` (vuelve a instalar de ser necesario) contiene para cada casilla:
...
Primero hacemos un *join* entre las tablas `sample_2006` y la tabla `strata_sample_2006` para tener toda la información en la misma tabla, esta información se guarda en la tabla `sample`.
...
Posteriormente se construye la función que calcula el *estimador de razón* para el partido deseado:
...
Usando la función anterior, calculamos el *estimador de razón* para cada partido:
...
Creamos una función para generar las muestras y calcular el **estimador de razón**.
...
Dados los **estimadores de razón** calculados, y el error estándard correspondiente, podemos calcular los intercalos para cada partido.
...
De la de información proveniente del **estimador de razón** se puede ver que que las elecciones de 2006 han sido de las más reñidas en los últimos años por la cercanía de votos entre el *PAN* y la alianza *PRD-PT-Convergencia*, al grado de la inestabilidad política que se generó posteriormente. Sin embargo, a pesar de la cercanía, la información en los intervalos de confianza no se entrelpa entre sí, lo que da evidencia suficiente para asegurar triunfo del Partido Accion Nacional, ver la gráfica de abajo.

```{r grafica, fig.height=4}
